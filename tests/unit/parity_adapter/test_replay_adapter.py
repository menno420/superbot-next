...
def test_full_corpus_reconstructs():
    """Every golden on disk yields a replayable case (479/479) — curated
    ...
    cases + the 5 WP-1 mining write-parity minted cases − 3 retired:
    sweep_cog.json, the deploy-ops `!cog` capture, plus
    ...
    # (D-0073 procedure) + 4 (browse-interaction, 2026-07-12) + 5 (WP-1
    # mining write-parity: equip/unequip/loadout save·apply·delete) − 3
    # retired (sweep_cog.json + sweep_query_logs.json + sweep_recent_errors.json
    # — parity.yml source.retired_goldens)
    assert golden_count == 479
