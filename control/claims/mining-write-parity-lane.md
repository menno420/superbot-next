...
- `wp3-depth-world-wear-writegoldens` · **write-parity WP-3 — geared descend/ascend/mineworld-reseed + repair/quickcraft write goldens [IN FLIGHT]** — geared-fixture goldens driving `mining.record_descend/ascend/reseed_world/repair/quick_craft`; retire `table:mining_world` + `table:mining_gear_wear` (remove face — the ported mine leg has NO wear tick, so the ADD face is unreachable and the WP-4 workshop repair/quickcraft terminals fold in per spec) + ratchet; adds two two-txn `lock_workshop_slot` concurrency regressions (quickcraft item-dup priority + repair double-spend) · area: parity/cases/, parity/goldens/mining/, parity/parity.yml, tests/integration/ · 2026-07-12T21:33:48Z
