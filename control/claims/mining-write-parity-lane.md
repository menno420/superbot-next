...
- `wp6-structures-craft-writegoldens` · **write-parity WP-6 — structure build (forge/home) PORT write golden [IN FLIGHT — PR pending]** — porting the oracle `mining_workflow.build_structure` (coin debit + material consume + `mining_structures` level raise in ONE txn) onto the audited `mining.build -> record_build` seam, flipping the forge/home 🔥 Build panel terminals from D-0043 pending to live handlers, minting the structure-build write golden(s) → retiring the LAST mining `guard-only-capture` row `table:mining_structures`; adds a `lock_structure_slot` advisory fence + a two-txn double-build concurrency regression. The FINAL slice — completes the write-parity lane. (craft `!build <gear>` stays a D-0043 pending terminal: the oracle command routes to `mining_workflow.craft`, whose product table `mining_inventory` is already covered.) · area: sb/domain/mining/, parity/, parity.yml, tests/ · 2026-07-13T00:00:00Z
