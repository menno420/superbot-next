...
async def run_app(env=None) -> int:  # noqa: PLR0911, PLR0915 — the boot script
    ...
    from sb.kernel.outbox.metrics import ALL_METRICS
    ...
    try:
        ...
        #    ALL_METRICS folds the outbox families into the base tuple so the
        #    durability spine is observable — the same canonical union
        #    tools/check_metric_cardinality validates; without the outbox
        #    families the relay's guarded counter bumps hit an
        #    unregistered-family KeyError that the guard silently discards.
        build_registry(ALL_METRICS)
