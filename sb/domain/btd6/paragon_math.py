"""BTD6 paragon catalogue + power model + reverse solver — the port of
shipped ``utils/btd6/paragon_math.py`` @7f7628e1: the 13-paragon catalogue
with medium base prices, the colloquial shorthand resolver, the degree
power-threshold cubic, the FORWARD power model (validated by the oracle
field-by-field against the live Paragon Calculator API), and the reverse
solver ("what inputs reach Degree X for the least cash / tiers / pops, or
balanced" — the inverse problem the API never exposed). The live-API
wrapper / reconciliation lane (``services/paragon_service.py`` HTTP,
caching, 429 taxonomy) stays a NAMED SUCCESSOR PORT (D-0046) — this
module is the API-replica local formula and never performs I/O.

Formula (per the API reference, confirmed against the live endpoint)::

    popsPower    = min(90000, floor((pops + income*4) / 180))
    upgradesPower= min(10000, upgrade_count * 100)            # tier cap 100
    cashPower    = min(60000, floor(cash_spent*(20000/base)
                                    + slider*(20000/(base*1.05))))
    ...
    threshold(D) = floor((50*D^3 + 5025*D^2 + 168324*D + 843000)/600),
                   threshold(100) = 200000
    degree       = max D in 1..100 with threshold(D) <= total_power

Pure, stdlib-only (``difficulty_costs`` is the same-band pure sibling)."""
...
CASH_POWER_NUMERATOR = 20_000  # power per $1 == CASH_POWER_NUMERATOR / base
...
# --- typed models (shipped verbatim, minus the live-API payload helper) ------
...
def validate_inputs(inputs: ParagonInputs) -> list[ParagonWarning]:
    ...
    if inputs.tier5_count > 0:
        if mode == "solo" and not paragon.is_dart:
            warnings.append(
                ParagonWarning(
                    "extra_t5_ignored",
                    "Extra T5s are ignored in solo for any paragon other "
                    "than Dart Monkey.",
                ),
            )
        else:
    ...
    if inputs.upgrade_count > MAX_UPGRADE_TIERS:
        warnings.append(
            ParagonWarning(
                "upgrades_capped",
                f"upgrade_count above {MAX_UPGRADE_TIERS} adds no power "
                "(10,000 cap).",
            ),
        )
    ...
    if inputs.slider_cash > slider_cap:
        warnings.append(
            ParagonWarning(
                "slider_clamped",
                f"slider_cash above ${slider_cap:,} (3.15x base) is "
                "clamped in-game.",
            ),
        )
    return warnings
...
def solve_requirements(
    ...
    *,
    ...
    result always reaches at least the target). Geraldo totems fill **only**
    the remainder that the capped axes cannot supply at their collective
    maximum — they never displace a capped axis (so "least cash/tiers/pops"
    stays a real minimum instead of collapsing to zero via unlimited totems).
    """
