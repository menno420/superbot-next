...
2. ~~**setup wizard interior**~~ — **DONE** (wizard-lifecycle slice, ORDER
   ...
   extras, health check + restart-resume bridge). Remaining named
   successors (smaller, now individually sliceable): the 9 remaining
   per-section flows · the suggestion Edit lane · the on-ready resume
   sweep (app-boot seam).
