...
(C4 · A-items/ai/hermes · btd6). D5 (e2e/live-guild harness), B8 (ux_lab wings)
and R (resilience/delivery/db) design docs were not yet on `main` when this
agenda was written — their decision areas are covered from the completeness
table + backlog framing and flagged `source doc pending`; the blocks upgrade
verbatim once those docs land.
...
| 24 | D5 e2e/live-guild harness shape | Fake in-process for PR tier; installed+secret-gated LIVE tier; shape-assert live, byte-assert headless | D5 harness slices `[?]` (source doc pending) |
| 25 | R resilience bounds | Capped exp-backoff; DLQ 30-day + manual replay; breaker at N fails; explicit refuse-write copy | R delivery-hardening slices `[?]` (source doc pending) |
...
- **Decision:** Port the admin-only ux_lab dev/diagnostic surface (9 wings) at all? If so, full behaviour or render-only for the 3 special wings (pil_cards / probe_bench / components_v2)? What priority, and what golden strategy?
- **Options:** A) Don't port (keep honest pending refusals) · B) Port render-only, special wings render-only too · C) Full port including image-bytes / live-error / CV2 special handling.
- **Recommendation:** **`[?]` LOW priority — port render-only if/when idle capacity exists, keep the 3 special wings render-only** (pil_cards = image bytes, probe_bench = version+date+live-errors, components_v2 = channel-side CV2 + external URLs all need special handling for marginal value). This is an **admin-only, zero-write dev tool** with **no user-facing gap** (the 9 wings are honest pending refusals fronting a fully-ported home), so it is genuinely deferrable — I cannot rank it above user-facing work without knowing how much *you personally* use the dev surface. The B8 design doc was not yet on `main`; this upgrades when it lands.
- **Unblocks:** B8 wing slices (foundation-first: port `utils/ux_patterns` + `ExhibitWingView`, then per-wing).
- **Source:** [completeness snapshot](../status/completeness-table-2026-07-18.md) B8 row; **B8 design doc pending**.
...
- **Decision:** (a) In-process Discord dependency — fake vs installed `discord.py`? (b) How/when does the LIVE tier run, and where do its secrets live? (c) Command set, pass/fail thresholds, assert bytes vs shape? (d) `verified_live` signer and guild hygiene?
- **Options:** (a) fake vs installed · (b) scheduled/dispatch + secret-gated vs on every PR · (c) byte-exact vs shape-only · (d) who/what signs a live run as trusted.
- **Recommendation:** **`[?]` (source doc pending)** — a reasonable default: **fake in-process for the PR tier** (deterministic, no secrets, byte-assertable against goldens) and **an installed-`discord.py` LIVE tier gated behind a secret + `workflow_dispatch`/nightly**, **shape-asserting** the live tier (a real guild's payloads carry non-deterministic ids/timestamps) while the headless tier stays byte-exact; a dedicated test guild with a teardown/hygiene step and a `verified_live` signer = the workflow identity that ran the live sweep. The D5 design doc was not yet on `main`; this upgrades to the doc's specifics when it lands.
- **Unblocks:** the D5 harness slices.
- **Source:** completeness snapshot D5 lane; **D5 design doc pending**.

### 25 — R resilience: delivery + DB hardening bounds
- **Decision:** Retry/backoff bounds, dead-letter retention + replay policy, breaker thresholds, refuse-write UX, and alerting for the delivery/DB resilience track.
- **Options:** numeric/policy choices per axis.
- **Recommendation:** **`[?]` (source doc pending)** — reasonable defaults to react to: **capped exponential backoff** (e.g. base 1 s, factor 2, cap ~5 min, bounded attempts) feeding the outbox dead-letter; **DLQ retention ~30 days with a manual replay tool** (never auto-replay a poison message); **breaker opens after N consecutive delivery failures** and half-opens on a timer; **refuse-write UX = an explicit, honest "write refused" reply** (never a silent success — matches the standing silent-success-is-a-bug posture); alerting rides the same `OperatorAlert` sink as #13. The R design doc was not yet on `main`; this upgrades when it lands.
- **Unblocks:** the R delivery-hardening slices.
- **Source:** resilience/delivery/db track; **R design doc pending**.
