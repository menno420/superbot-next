# superbot-next — routines & wake-chain doctrine

> ⚠️ **RETIRED (2026-07-17) — autonomy wake-chain wind-down.** The self-wake /
> pacemaker / dead-man-cron / failsafe trigger doctrine below is retired ahead of
> the Claude Code Projects EAP going read-only Tue 2026-07-21; the Project will be
> recreated fresh with **no standing wake chain**. **Do not arm or keep any
> pacemaker / failsafe / poke trigger.** Residual live triggers must be disarmed
> via the routines UI (owner action) — see `control/status.md` § ⚑ needs-owner.
> This doc is kept as historical record only.
